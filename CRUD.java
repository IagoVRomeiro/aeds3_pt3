...
public class CRUD {
    ...
    private static int versaoCompressao = 1;
    ...
    public static void CRUD(TreeBplus arvore, HashEstendido hash) throws IOException {
        ...
        while (true) {
            ...
            System.out.println("6. Compactar base de dados (Huffman)");
            System.out.println("7. Descompactar base de dados (Huffman)");
            ...
            switch (opcao) {
                ...
                case 6 -> {
                    try {
                        String arquivoCompactado = String.format("dataset/capitulosHuffmanCompressao%d.db", versaoCompressao);
                        ...
                        System.out.printf("Ganho de compressao: %.2f%%\n", ganho);

                        versaoCompressao++;
                    } catch (IOException e) {
                        System.out.println("Erro na compactação: " + e.getMessage());
                    }
                }
                case 7 -> {
                    ...
                    String arquivoCompactado = String.format("dataset/capitulosHuffmanCompressao%d.db", versao);
                    File f = new File(arquivoCompactado);
                    if (!f.exists()) {
                        System.out.println("Arquivo compactado nao encontrado!");
                        break;
                    }

                    try {
                        long inicio = System.currentTimeMillis();
                        Huffman.descomprimir(arquivoCompactado);
                        long fim = System.currentTimeMillis();
